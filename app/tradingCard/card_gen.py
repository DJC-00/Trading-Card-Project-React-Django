...
class TCGCard:
    ...
    def get_stats():
        ...
        for i in range(0,3):
            ...
            stats.append(finalSum)
        return stats

    def get_rarity(self):
        statAvg = ((sum(self.stats))/4)
        if statAvg == 10:
            return ("Masterpiece")
        elif statAvg > 8:
            return ("Legendary")
...
def getName(urlList):
    names = ""
    service = FirefoxService(executable_path="/tcg-project-folder-2/app/app/tradingCard/webdriver/geckodriver-v0.31.0-linux64.tar.gz")
